                            ...
                            allInputs.forEach((inp, idx) => { 
                                if(!inp.disabled) { 
                                    ...
                                    inp.addEventListener('compositionupdate', function(e) {
                                        // Đang nhào nặn chữ (Telex/Pinyin), CỨ ĐỂ NGUYÊN cho bộ gõ làm việc, KHÔNG cắt xén gì cả!
                                        // Giao diện có thể hiện 2-3 chữ tạm thời trong ô, kệ nó.
                                    });

                                    inp.addEventListener('compositionend', function(e) { 
                                        isComposing = false; 
                                        // Khi đã chốt chữ (ví dụ ra chữ 'é' hoặc chữ Hán)
                                        if(inp.value) {
                                            // Chỉ lấy ký tự CUỐI CÙNG (phòng khi bộ gõ nhét cả cụm dài)
                                    ...
                                    inp.addEventListener('input', function(e) { 
                                        // Nếu không phải đang gộp chữ (gõ tiếng Anh bình thường)
                                        if(!isComposing && inp.value) { 
                                            inp.value = inp.value.slice(-1).toUpperCase();
                                            jumpToNext(idx);
                                        } 
                                    });
                                    
                                    // Tách logic nhảy ô ra một hàm riêng cho gọn
                                    const jumpToNext = (currentIdx) => {
                                        if(currentIdx < allInputs.length - 1) { 
                                            let nextIdx = currentIdx + 1; 
                                            let nextInp = allInputs[nextIdx]; 
                                            while(nextInp && nextInp.disabled && nextIdx < allInputs.length - 1) { 
                                                nextIdx++; 
                                                nextInp = allInputs[nextIdx]; 
                                            } 
