let availableRealms = []; 
        ...
        function fetchLessonsFromFirebase() {
            ...
            db.collection('realms').doc(currentRealm).collection('lessons').orderBy('created', 'asc').onSnapshot(snap => {
                ...
                snap.forEach(doc => {
                    ...
                    htmlLib += `<div class="lib-card">${deleteBtn}<h3>${data.name}</h3><p style="font-size:11px;">Số lượng: ${data.vocab.length} từ</p><button class="btn-run" style="margin-top:10px;" onclick="viewCard('${btoa(unescape(encodeURIComponent(data.html)))}')">▶ VÀO HỌC</button></div>`;
        ...
        function executeAntiCheatPunishment() {
            ...
            // Dùng .set() an toàn, không đè mất p1/p2
        ...
        function forceEndMatch() {
            ...
            isUnderSurveillance = false; // Tạm tắt giám sát để tránh lỗi popup confirm
            ...
            if(!confirm("Bạn có chắc chắn muốn kết thúc ép buộc trận đấu này? Thao tác này sẽ giải tán võ đài và giải phóng cả 2 tuyển thủ khỏi tình trạng bị kẹt.")) {
                isUnderSurveillance = tempSurveillance; // Bật lại nếu bấm Hủy
                return;
            }
        ...
        function surrenderMatch() {
            let tempSurveillance = isUnderSurveillance;
            isUnderSurveillance = false; // Tạm tắt giám sát để không bị Anti-cheat phạt lộn do confirm()
            ...
            if(confirm("Bạn có chắc chắn muốn rút lui khỏi trận này? Hệ thống sẽ xử thua!")) {
                ...
                rtdb.ref(`active_pvp_match/${currentRealm}`).once('value').then(snap => {
                    ...
                    if(m && m.status !== 'finished') {
                        ...
                        if(m.stage && m.match_idx !== undefined && m.league) {
                            ...
                            // Ghi trực tiếp winner để bảo toàn dữ liệu p1/p2
                ...
                isUnderSurveillance = tempSurveillance; // Bật lại nếu chọn Hủy
            }
        }
        ...
        function viewCard(base64) { document.getElementById('modalFrame').srcdoc = decodeURIComponent(escape(atob(base64))); document.getElementById('previewModal').classList.add('active'); }
        ...
        // CHỨC NĂNG PVP HOÀN THIỆN ĐÃ GÁN CURRENT REALM
        async function adminStartPvP(league, stageKey, matchIndex, p1Name, p2Name) { 
            ...
            window.isSpectating = true; // Quản trị viên bắt đầu theo dõi
    ...
    <div id="summary-section" class="hidden w-full max-w-md mx-auto relative">
        <div class="bg-white rounded-2xl shadow-lg p-6 mb-6 border border-gray-100">
            ...
            <div id="summary-list" class="max-h-96 overflow-y-auto pr-2 space-y-2">
                </div>
        </div>
