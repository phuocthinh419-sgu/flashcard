    ...
    <script>
        ...
        function submitSpelling() {
            ...
            function formatWords(str) {
                let arr = str.split('/');
                let res = [];
                for(let i=0; i<arr.length; i++) res.push(arr[i].trim());
                res.sort();
                return res.join('/');
            }
...
function toggleStoreMenu() {
    ...
    if (dropdown.style.display === 'none' || dropdown.style.display === '') {
        // Mở ra: Ép dùng flex dọc để không bị vỡ layout
...
function switchTab(tabId) { 
    try {
        ...
        // --- XỬ LÝ LÀM SÁNG NÚT BẤM KHI CHỌN ---
        if (tabId === 'shop') {
            document.getElementById('nav-shop-main').classList.add('active');
        ...
        // Tự động xổ Cửa hàng nếu Bệ hạ chuyển tab mà nó đang đóng
...
// =========================================================
// 📦 TẠO KHUNG ĐỔ DỮ LIỆU CHỢ ĐEN (MÔ PHỎNG)
// =========================================================
